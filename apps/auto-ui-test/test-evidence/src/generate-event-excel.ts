// 외부 라이브러리 없는 클라이언트 사이드 이벤트 로그 XLSX 생성
...
function eventDesc(e: EventLogItem): string {
  if (e.eventType === "click") return `[${e.targetLabel}] 클릭`;
  if (e.eventType === "input") return e.value ? `[${e.targetLabel}] 입력: "${e.value}"` : `[${e.targetLabel}] 입력`;
  return `${e.url} 이동`;
}
...
function coverSheet(p: EventExcelPayload) {
...
function revisionSheet(p: EventExcelPayload) {
  return sheet([12, 20, 55, 15], [
    row(1, ["버전", "개정일자", "개정내용", "개정자"].map((h, i) => tc(1, i + 1, h, 2)), 22),
    row(2, [tc(2, 1, "V1.0", 1), tc(2, 2, p.capturedAt ?? "", 1), tc(2, 3, "최초 작성", 1), tc(2, 4, p.author, 1)], 20),
  ], []);
}
...
function eventLogSheet(p: EventExcelPayload) {
  ...
  return sheet(colWidths, rows, ms);
}
...
function evidenceSheet(events: EventLogItem[], imgs: Img[]) {
  const withScreenshot = events
    .map((e, i) => ({ e, img: imgs[i] }))
    .filter(({ e }) => !!e.screenshot);

  const rows = [
    row(1, [tc(1, 1, "이벤트 결과 증빙", 4)], 28),
    row(2, [tc(2, 1, "순번", 2), tc(2, 2, "이벤트 설명 / 캡처 화면", 2)], 22),
  ];
  const ms = ["A1:I1", "B2:I2"];
  const areaW = 8 * IMG_COL_WIDTH * IMG_COL_PX;
  let cur = 3;

  withScreenshot.forEach(({ e, img }) => {
    const { w, h } = img;
    ...
    for (let r = cur; r <= end; r++) {
      const cells: string[] = r === cur ? [tc(r, 1, pad2(e.seq), 3)] : [];
      for (let col = 2; col <= 9; col++) {
        const s = borderStyle(r === cur, r === end, col === 2, col === 9);
        if (s) cells.push(ec(r, col, s));
      }
  ...
  return sheet([14, ...Array(8).fill(IMG_COL_WIDTH)], rows, ms, withScreenshot.length ? "rId1" : undefined);
}

function drawingXml(events: EventLogItem[], imgs: Img[]) {
  const withScreenshot = events
    .map((e, i) => ({ e, img: imgs[i] }))
    .filter(({ e }) => !!e.screenshot);
  ...
  const areaW = 8 * IMG_COL_WIDTH * IMG_COL_PX;
  let cur = 3;
  const anchors = withScreenshot.map(({ img }, idx) => {
    const { w, h } = img;
    const numRows = Math.max(10, Math.ceil(areaW * (h / Math.max(w, 1)) / (IMG_ROW_HEIGHT_PT * PT_TO_PX)));
    const end = cur + numRows - 1;
    const xml = `<xdr:twoCellAnchor editAs="oneCell"><xdr:from><xdr:col>1</xdr:col><xdr:colOff>0</xdr:colOff><xdr:row>${cur - 1}</xdr:row><xdr:rowOff>0</xdr:rowOff></xdr:from><xdr:to><xdr:col>9</xdr:col><xdr:colOff>0</xdr:colOff><xdr:row>${end}</xdr:row><xdr:rowOff>0</xdr:rowOff></xdr:to><xdr:pic><xdr:nvPicPr><xdr:cNvPr id="${idx + 1}" name="Event ${idx + 1}"/><xdr:cNvPicPr/></xdr:nvPicPr><xdr:blipFill><a:blip r:embed="rId${idx + 1}"/><a:stretch><a:fillRect/></a:stretch></xdr:blipFill><xdr:spPr><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></xdr:spPr></xdr:pic><xdr:clientData/></xdr:twoCellAnchor>`;
    cur = end + 1;
    return xml;
  });
...
const SHEET_NAMES = ["겉표지", "개정이력", "이벤트 로그", "공통체크리스트", "이벤트 결과 증빙"];

export function makeEventFileName(p: Pick<EventExcelPayload, "serviceCode" | "screenId" | "screenName">) {
  return `MAL_${p.serviceCode || "서비스코드"}_AC02(단위테스트케이스결과서)_UT_${p.screenId || "화면ID"}_${p.screenName || "화면명"}_V1.0.xlsx`;
}

export function generateEventExcel(payload: EventExcelPayload): Promise<Uint8Array> {
  const p = {
    ...payload,
    capturedAt: payload.capturedAt ?? todayStr(),
    events: payload.events.map((e, i) => ({ ...e, seq: i + 1 })),
  };

  const eventsWithScreenshot = p.events.filter(e => !!e.screenshot);
  const imgs = p.events.map(e => e.screenshot ? parseImg(e.screenshot) : null);
  const screenshotImgs = eventsWithScreenshot.map(e => parseImg(e.screenshot!));
  ...
  const hasImgs = eventsWithScreenshot.length > 0;
  const files: { name: string; data: string | Uint8Array }[] = [
    { name: "[Content_Types].xml", data: `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/>${screenshotImgs.some(i => i.ext === "png") ? '<Default Extension="png" ContentType="image/png"/>' : ""}${screenshotImgs.some(i => i.ext === "jpeg") ? '<Default Extension="jpeg" ContentType="image/jpeg"/>' : ""}<Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml"/>${SHEET_NAMES.map((_, i) => `<Override PartName="/xl/worksheets/sheet${i + 1}.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/>`).join("")}${hasImgs ? '<Override PartName="/xl/drawings/drawing1.xml" ContentType="application/vnd.openxmlformats-officedocument.drawing+xml"/>' : ""}</Types>` },
    ...
    { name: "xl/worksheets/sheet3.xml", data: eventLogSheet(p) },
    { name: "xl/worksheets/sheet4.xml", data: checklistSheet() },
    { name: "xl/worksheets/sheet5.xml", data: evidenceSheet(p.events, imgs as Img[]) },
  ];
  ...
  if (hasImgs) {
    ...
    files.push({ name: "xl/drawings/drawing1.xml", data: drawingXml(p.events, imgs as Img[]) });
    files.push({ name: "xl/drawings/_rels/drawing1.xml.rels", data: rels(screenshotImgs.map((img, i) => ({ id: `rId${i + 1}`, type: "http://schemas.openxmlformats.org/officeDocument/2006/relationships/image", target: `../media/image${i + 1}.${img.ext}` }))) });
    screenshotImgs.forEach((img, i) => files.push({ name: `xl/media/image${i + 1}.${img.ext}`, data: img.bytes }));
  }
